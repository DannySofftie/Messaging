...
try {
    ...
    $chatMsgFetchQuery = $conn->prepare(
        "SELECT * from (select * from chat_messages as t where (friend_id = :friend_id or friend_id= :userid) and (my_id= :friend_id or my_id= :userid) 
        order by chat_time desc limit 9)  as t order by chat_time asc"
      );
